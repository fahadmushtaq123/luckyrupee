// ============================================================
// lib/screens/draw/draw_detail_screen.dart
// Draw detail — entry, skill question, countdown, live feed
// ============================================================
...
class _DrawDetailScreenState extends ConsumerState<DrawDetailScreen> {
  ...
  Future<void> _fetchQuestion() async {
    ...
    try {
      final q = await ref.read(authServiceProvider).getSkillQuestion(widget.drawId);
      setState(() { _question = q; _selectedAnswer = null; });
    } catch (_) {
      // If question fails, still allow viewing draw
    } finally {
  ...
  Future<void> _enterDraw(DrawModel draw) async {
    if (_question == null) return _fetchQuestion();
    ...
    try {
      final result = await ref.read(authServiceProvider).enterDraw(
        drawId:       widget.drawId,
        entries:      _entryCount,
        skillAnswer:  _selectedAnswer!,
      ...
      if (result.success) {
        ...
        ref.invalidate(drawDetailProvider(widget.drawId));
  ...
  void _showSuccessSheet(EntryResult result) {
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('${result.entries} ${result.entries == 1 ? 'entry' : 'entries'} confirmed',
            style: const TextStyle(color: kMuted, fontSize: 16)),
          const SizedBox(height: 8),
          Text('New balance: PKR ${result.newBalance.toStringAsFixed(2)}',
            style: const TextStyle(color: kGold, fontSize: 14, fontWeight: FontWeight.w600)),
          const SizedBox(height: 24),
          GoldButton(label: 'Awesome! 🚀', onTap: () {
            Navigator.pop(context);
            _fetchQuestion(); // New question for next entry
          }),
          const SizedBox(height: 12),
        ]),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: drawAsync.when(
        ...
        error: (e, _) => Center(
          child: EmptyState(emoji: '⚠️', title: 'Draw not found', subtitle: e.toString())),
      ),
    );
  }
        ...
        SliverAppBar(
          ...
          leading: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: kBg.withOpacity(0.8), shape: BoxShape.circle),
          ...
          flexibleSpace: FlexibleSpaceBar(
            background: CachedNetworkImage(
              ...
              errorWidget: (_, __, ___) =>
                Container(color: kSurface, child: const Center(child: Text('🎁', style: TextStyle(fontSize: 72)))),
            ),
          ),
        ),
              ...
              children: [
                  ...
                  children: [
                    ...
                    ClipRRect(
                      ...
                      child: LinearProgressIndicator(
                        ...
                        valueColor: AlwaysStoppedAnimation(draw.isAlmostFull ? kRed : kGreen),
                      ),
                    ),
                  ],
                ),
                ...
                Container(
                  ...
                  child: Column(children: [
                    Row(
                      ...
                      children: [
                        ...
                        Row(children: [
                          _CountButton(
                            icon: '-',
                            onTap: _entryCount > 1 ? () => setState(() => _entryCount--) : null,
                          ),
                          Padding(
                    ...
                    const SizedBox(height: 12),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                      const Text('Cost', style: TextStyle(color: kMuted, fontSize: 13)),
                    ...
                    if (draw.userEntries > 0)
                      Padding(
                        ...
                        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          const Text('Your current entries', style: TextStyle(color: kMuted, fontSize: 13)),
                          Text('${draw.userEntries}',
                ...
                GoldButton(
                  label: _entered
                    ? 'Enter Again — PKR ${totalCost.toStringAsFixed(0)}'
                    : 'Enter Draw — PKR ${totalCost.toStringAsFixed(0)}',
                  loading: _loading,
                ...
                const SizedBox(height: 12),
                const Center(
                  child: Text('🔒 Cryptographically fair • Results published publicly',
...
class _SkillQuestionWidget extends StatelessWidget {
  ...
  const _SkillQuestionWidget({required this.question, this.selected, required this.onSelect});
          ...
          ...options.map((opt) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isSelected ? kGold.withOpacity(0.15) : kSurface,
                  borderRadius: BorderRadius.circular(10),
                  border: Border.all(
                ...
                child: Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: isSelected ? kGold : kBorder.withOpacity(0.5),
                    ),
                    child: Center(
                      child: Text(opt.$1.toUpperCase(),
                        style: TextStyle(
                          fontSize: 11, fontWeight: FontWeight.w800,
                          color: isSelected ? kBg : kMuted,
                        )),
                    ),
                  ),
...
class _CountdownWidgetState extends State<_CountdownWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        Text('⏱ Ends in', style: TextStyle(color: kMuted, fontSize: 10)),
        Text(
          h > 0 ? '${h}h ${m}m' : '${m}m ${s.toString().padLeft(2, '0')}s',
          style: TextStyle(
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: onTap != null ? kGold.withOpacity(0.4) : kBorder),
        ),
        child: Center(
          child: Text(icon,
            style: TextStyle(
